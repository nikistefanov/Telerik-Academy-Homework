function createImagesPreviewer(selector, items) {
    ...
    input.addEventListener('keyup', function (ev) {
        ...
        var liChilder = listOfItems.getElementsByTagName('li');

        for (var i = 0, len = liChilder.length; i < len; i += 1) {
            var currentLi = liChilder[i];
            var header = currentLi.firstElementChild.innerText;

            if (header.toLowerCase().lastIndexOf(text.toLowerCase()) >= 0) {
